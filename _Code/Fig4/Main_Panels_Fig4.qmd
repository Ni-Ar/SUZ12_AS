...
We performed histone PTMs DIA mass spectrometry and I analysed the raw files with EpiProfile v2.1 [@Yuan2018]. Here I parse the EpiProfile output and analyse the PTMs values with DEP [@Zhang2018].

**Acknowledgements**: I'd like to thank Zuo-Fei Yuan and Simone Sidoli from the [Ben Garcia lab](https://twitter.com/garcialabms?lang=en) for their help with data analysis and sample preparation respectively. Also a big thank you to Cristina Chiva from the [CRG/UPF proteomics facility](https://www.crg.eu/en/programmes-groups/crgupf-proteomics-unit).
...
```{r, set paths and directories, warning = FALSE, message = FALSE}
...
```{r, process and tidy up the EpiProfile output.}
data_dia_auto |>
...
# Differential abundance analysis of histone PTMs with DEP

Use the area under the peak as counts to be analysed using DEP framework.

```{r, import all EpiProfile data, warning = FALSE, message = FALSE}
...
::: {.callout-caution collapse="true" title="Note on the MS peaks areas"}
One small caveat in this analysis is that the areas under the peaks are very large numbers or at least larger than `r .Machine$integer.max` which is the highest number `R` can store as integer. To convert the areas to integers from numeric type one could use the package `bit64` that stores integers in 64 bits instead of the default 32 bit. My solution was however simpler, I divided the all values by `1000` to scale down by 3 order of magnitude. I then convert the dataframe to matrix.
:::
...
The data frame `data_unique` contains `r ncol(data_unique)` columns (of which `r ncol(data_unique) - 4` are samples) and `r nrow(data_unique)` rows corresponding to the identified histone PTMs.
...
```{r, check frequency of histone PTMs across all samples, fig.height = 4, fig.width = 5}
...
```{r, mean SD plot, fig.height = 4, fig.width = 5}
...
```{r, check normalisations, fig.height = 10, fig.width = 8}
...
```{r, missing values plot, fig.height = 6, fig.width = 4.5}
...
## Test for significance against WT control

Test every sample versus WT control

```{r, test diff, warning = FALSE, message = FALSE}
...
```{r, plot distances between samples, fig.height = 5, fig.width = 5}
...
```{r, plot heatmap, warning = FALSE, fig.height = 5, fig.width = 5}
...
### Volcano plot rescue SUZ12-L vs SUZ12-S

Plot rescues.

```{r, plot rescues as volcano, fig.width = 4.5, fig.height = 5 }
ggplot(res_KOrS) +
  aes(x = ratio, y = -log10(p.val), fill = Direction, size = WT_Mean_Area ) +
  geom_point(shape = 21, stroke = 0.2) +
...
## Protein PSI quantification

```{r, import WB quantifications of the SUZ12 bands}
...
```{r, plot WB quantifications, fig.width = 1.85, fig.height = 2}
