...
theme_bw(base_family = 'Arial', base_size = 6) +
  theme(axis.line = element_line(colour = "black", linewidth = 0.1),
        ...
        legend.position.inside = c(0.89, 0.96),
        ...
        plot.margin = margin(t = 0, b = 0, l = 0, r = 0, unit = 'mm')
        ) -> gradients_theme
...
ggplot(data = fractionation_wb) +
  ...
  scale_x_continuous(breaks = seq(2, 24, 2), limits = c(2, 24),
                     expand = expansion(add = 0.5, mult = 0) ) +
...
Make also a smaller figure removing the first and second as well as the penultimate and last fractions, a zoom-in version of the plot. Also remove EZH2 for simplicity

```{r, gradient zoomed in, message = FALSE, warning = FALSE, fig.width = 1.9, fig.height = 2.05}
subset(fractionation_wb, Protein != 'EZH2') |>
  mutate(Protein = factor(Protein, levels = c('SUZ12', 'JARID2', 'AEBP2', 'EPOP', 'MTF2') ) ) |>
  ggplot() +
    aes(x = Fraction, y = Intensity) +
    facet_grid(rows = vars(Protein) ) +
    geom_line(aes( color = line), linewidth = 0.25, linetype = 'solid') +
    geom_point(aes(fill = line), size = 1.25, shape = 21, stroke = 0.2, colour = 'black') +
    scale_x_continuous(breaks = seq(2, 24, 2), limits = c(6, 18),
    ...
    gradients_theme +
    theme(legend.position.inside = c(0.16, 0.95),
          axis.title.x = element_text(margin = margin(t = -0.95, unit = 'mm') )) -> p_wt_dex_frac_zoom
p_wt_dex_frac_zoom
```
...
```{r, for loop to calcualte distances to RBBP4 residues, message = TRUE, warning = FALSE}
pdb_rank <- c(1:5)
list_df <- list()
list_p_anno <- list()
...
Make a plot with all distances to RBBP4 negatively charged residues for each of the 5 ranked model as boxplot
...
Make a figure with only the first best model, where best means lower pLDDT.
...
I also calculate the distance between the SUZ12 ZnB residues important for AEBP2 and JARID2 interaction used generate PRC2.1 only forming cells with the artificial introduction of an A2J2mut1 reported previously [@Youmans2021].

```{r, ZnB residues}
ZnB_residue_num <- c(95:106)
```

Calculate distance for all ranked models and test for significance.

```{r, ZnB distance calculation, warning = F}
pdb_rank <- c(1:5)
list_df <- list()
list_p_anno <- list()
for (p in 1:length(pdb_rank)) {
  ...
  ## extract PDB coordinates of the SUZ12 ZnB domain
  # inclusion PDB
  group2_coord_inc <- pdb2coord_df(pdb = pdb_inc, 
                                   residue_num = ZnB_residue_num, 
                                   chain_letter = 'B', atom_type = 'CA')
  ...
  dist_df <- rbind(dist_inc, dist_exc) |>
    ...
    subset(!to_resno %in% c(SUZ12_residue_num - 23, SUZ12_residue_num) ) |>
    # Add residue distance pair column
    # add 23 to the short to create a matching pair
    mutate(AA_pair = case_when(structure == 'SUZ12-L' ~ paste0(resno, "-", to_resno),
                               structure == 'SUZ12-S' ~ paste0(resno + 23, "-", to_resno) ) )
   # -- 5 -- Significance test ----
  # Calculate p-values and format them for a figure in ggplot
  dist_df |>
    wilcox_test(dist ~ structure, ref.group = "SUZ12-L",
  ...
  ggplot(dist_df) +
    ...
    scale_y_continuous(limits = c(0, NA),
                       ...
                       n.breaks = 8) +
    labs(title = paste0('CF pdb rank #', pdb_rank[p]),
         y = "Distance SUZ12 R196 to ZnB domain (Å)") +
    coord_cartesian(clip = 'off') + colabfold_distances_theme -> p_BoxPlot_Dists
    
  # ggsave(filename = paste0("CF_SUZ12_ZnB_Dist_rank", pdb_rank[p], ".pdf"),
  #        bg = NULL, plot = p_BoxPlot_Dists, device = cairo_pdf,
  #        symbolfamily = "Helvetica", pointsize = 5, path = pdf_dir_fig3,
  #        width = 3, height = 6.5, units = "cm")
...
```{r, combine the list of dataframes for Arg196 and ZnB}
dist_df_5_pdbs <- do.call('rbind', list_df)
p_anno_df_5_pdbs <- do.call('rbind', list_p_anno)
...
Make a plot with all distances to ZnB domain for each of the 5 ranked model as boxplot

```{r, plot all distances of SUZ12 Arg196 and ZnB residues, fig.width = 4, fig.height = 2, warning = FALSE, message = FALSE}
ggplot(dist_df_5_pdbs) +
  aes(x = structure, y = dist, fill = structure) +
  facet_wrap(~model_rank, nrow = 1)+
  geom_boxplot(show.legend = F, linewidth = 0.15, outlier.shape = NA) +
  ...
  scale_y_continuous(limits = c(0, NA),
                     ...
                     n.breaks = 8) +
  scale_fill_manual(values = c('SUZ12-S' = '#f07e19', 'SUZ12-L' = '#7b67ab')) +
  labs(y = "Distance SUZ12 R196 to ZnB domain (Å)") +
  coord_cartesian(clip = 'off') +
  colabfold_distances_theme -> p_BoxPlot_AllModels_SUZ12_RKR_ZnB_Dist
```

Save all models to pdf.

```{r, save all models ZnB}
ggsave(filename = paste0("CF_SUZ12_ZnB_EuclDist_rank1-", length(pdb_rank), ".pdf"), 
       plot = p_BoxPlot_AllModels_SUZ12_RKR_ZnB_Dist, 
       device = cairo_pdf, symbolfamily = "Helvetica", pointsize = 5, path = pdf_dir_fig3, 
       width = 3*length(pdb_rank)+2, height = 6.5, units = "cm")  
```
...
```{r, best model figure S3F, fig.width = 1, fig.height = 2, warning = FALSE, message = FALSE}
subset(dist_df_5_pdbs, model_rank == 1) |>
  ggplot() +
  aes(x = structure, y = dist, fill = structure) +
  geom_boxplot(show.legend = F, linewidth = 0.15, outlier.shape = NA) +
...
```{r, save figure S3F}
ggsave(filename = "FigS3F_SUZ12_ZnB_EuclDist_rank1_Boxplot.pdf", 
       plot = p_BestModel_All_Dists,  path = pdf_dir_fig3, 
       device = cairo_pdf,
       width = 2.0, height = 4.10, units = "cm")  
```
...
Combine both biological replicates into one plot in case someone wants it.
