...
library(MSnbase) # BiocManager::install("MSnbase")
library(UniprotR) # BiocManager::install("UniprotR")
library(DEP) # BiocManager::install("DEP")
library(knitr)
library(UpSetR)
...
plot_pep_cov <- function(df_res, AA_exon_start, AA_exon_end, Param_plot = 'Area') {
  ...
  subset(df_res, Parameter == Param_plot) |>
    ...
    geom_vline(xintercept = AA_exon_start, linewidth = 0.2) +
    geom_vline(xintercept = AA_exon_end, linewidth = 0.2) +
...
saint_path_WT_dEx4 <- file.path(tbl_dir_fig2, 
                                "Suz12_IP-MS_mESCs_WT_vs_dExon4_Saint.xlsx")
saint_path_KO_Rescues <- file.path(tbl_dir_fig2,
                                   "Suz12_IP-MS_mESCs_Suz12KO_vs_Rescues_Saint.xlsx")
...
```{r, plot figure 2D, fig.height = 2, fig.width = 2.5}
ggplot(PRM_areas)+
...
This mzTAB file was deposited on PRIDE under the accession code [PXD046660](https://www.ebi.ac.uk/pride/archive/projects/PXD046660).
...
```{r, normalise using SUZ12 length}
...
```{r, prepare for plot of stoichiometry ratio, warning = F, message = F}
...
The mzTAB file (generate with the `proteomicslfq` pipeline) was deposited on PRIDE under the accession code [PXD046660](https://www.ebi.ac.uk/pride/archive/projects/PXD046660).
...
```{r, advanced missing value imputation dataset 2}
...
```{r, show impotation dataset 2, fig.height = 12, fig.width = 7}
...
```{r, show normalisation dataset 2, fig.height = 10, fig.width = 6, message = F, warning=FALSE}
plot_normalization(data_norm, data_imp_knn, data_imp_mixed) +
  scale_fill_manual(values = c("KOrS" = "darkorange1", "KOrL" = "mediumpurple3", "KO" = "gray84")) +
  theme(axis.text = element_text(color = 'black'),
        legend.position = "bottom",
...
```{r, show PCA dataset 2, warning=FALSE, message=FALSE}
plot_pca(data_imp_mixed, x = 1, y = 2, n = 500, point_size = 4) +
  scale_colour_manual(values = c("KOrS" = "darkorange1", "KOrL" = "mediumpurple3", "KO" = "gray84")) +
  theme(plot.background = element_blank(), 
        plot.title = element_blank(),
...
```{r, get differentially enriched proteins dataset 2, warning = FALSE, message = F}
# DE analysis on no, MinProb, man, knn, and MLE imputations
...
```{r, show number of significant proteins in dataset 2}
# Number of significant proteins
objects <- c("no_imputation_res", "minprob_imputation_res", 
             "man_imputation_res", "knn_imputation_res", 
             "mle_imputation_res", "zer_imputation_res", 
...
```{r, set significance threshold in rescue IP-MS vs KOrL}
data_imp_mixed |>
  test_diff(type = "control", control = "KOrL") |>
...
```{r, set significance threshold in rescue IP-MS vs KO}
data_imp_mixed |>
  test_diff(type = "control", control = "KO") |>
  add_rejections(alpha = 0.05, lfc = log2(1.25)) |>
  get_results() |>
...
```{r, get labels ready for volcano plot of rescues}
...
## Check overlap between SUZ12-IP and FLAG-IP

Check how many proteins are in each IP-MS experiments and how many are shared or unique to each experiment. 
For this analysis I use the protein "UniProtID" (column name `ID`) as the identifier. One could also change the `$ID` to `$name` to a more human readable protein identifiers.

```{r, print all num proteins idenfied}
message(length(res_WT_Dexon4$ID), '\tproteins idenfied with SUZ12 IP-MS')
message(length(res_KO_rescues$ID), '\tproteins idenfied with FLAG IP-MS')
```

Make a list of all identified proteins for the upset plot

```{r, make list of all proteins for upset plot}
all_identified_proteins <- list(res_WT_Dexon4$ID, res_KO_rescues$ID)
...
```{r, make upset plot intersection all proteins identified}
...
```{r, show_upset_plot_all, fig.width = 5.5, fig.height = 4.5}
prot_intersection
```
...
```{r, save upset plot all interactions as jpeg, message = FALSE, warning = FALSE}
...
```{r, make a dataframe with overlaps}
suz12_flag <- intersect(res_WT_Dexon4$ID, res_KO_rescues$ID)
suz12_specific <- res_WT_Dexon4$ID[!res_WT_Dexon4$ID %in% suz12_flag]
flag_specific <- res_KO_rescues$ID[!res_KO_rescues$ID %in% suz12_flag]
...
Set thresholds for defining "enriched" proteins.
...
Identify "enriched proteins" by having |ratio| >= `r ratio_thrshld` and *P*-value =< `r pval_thshld` (lose stringency).

```{r, print num proteins idenfied}
wt_enriched <- subset(res_WT_Dexon4, Dexon4_vs_WT_ratio <= -ratio_thrshld &
                          Dexon4_vs_WT_p.val <= pval_thshld )$ID
...
KOrL_enriched <- subset(res_KO_rescues, KOrS_vs_KOrL_ratio <= -ratio_thrshld &
                          KOrS_vs_KOrL_p.val <= pval_thshld )$ID
...
```{r, make list for upset plot of enriched proteins}
...
```{r, make upset plot enriched proteins}
...
```{r, show_upset_plot_enriched_proteins, fig.width = 5.5, fig.height = 4.5}
prot_overlap
```

Save this upset plot to pdf.

```{r, save upset plot enriched proteins intersections as pdf, message = FALSE, warning = FALSE}
...
Save this upset plot jpeg.
```{r, save upset plot enriched proteins intersections as jpeg, message = FALSE, warning = FALSE}
...
```{r, make a dataframe with overlaps of enriched proteins}
...
```{r, pretty print overlap}
#| tbl-cap: "Overlap of significant proteins between SUZ12-IP and FLAG-IP"
#| tbl-colwidths: [60,40]
kable(intersections)
```
...
```{r, export isect all prot}
write_excel_csv2(x = all_prot_intersection, 
                 file = file.path(tbl_dir_fig2, 'SUZ12_FLAG_IPs_all_proteins_intersection_UniProtID.xls'),
                 na = "NA", append = F, col_names = T, delim = ";", quote = "all", eol = "\n")
```

Export overlap dataframe of enriched proteins. 

```{r, export overlap dataframe}
write_excel_csv2(x = intersections, 
                 file = file.path(tbl_dir_fig2, 'SUZ12_FLAG_IPs_enriched_proteins_overlaps_UniProtID.xls'),
                 na = "NA", append = F, col_names = T, delim = ";", quote = "all", eol = "\n")
```
...
Only part of the exon is covered but we can't claim major differences in peptides abundance.
...
Combine both exon annotation into one plot

```{r, Plot_WT_Dex_EZH2_exon_4_14_peptide_coverage, fig.width = 6, fig.height = 1.8}
plot_pep_cov(df_res = df_ezh2_ex4_wt, AA_exon_start = 83, AA_exon_end = 121) +
  labs(title = 'EZH2 exon 4 & exon 14 (SUZ12-IP)') + 
  geom_vline(xintercept = 516) +
  geom_vline(xintercept = 558) -> p_EZH2_ex4_ex14_cov
```

Save to pdf.

```{r, Save WT Dex IP EZH2 exon 4 and 14 peptide coverage}
ggsave(filename = "Rev3_EZH2ex4-14_WT_Dex4_IP_Peptides_Coverage.pdf", 
       plot = p_EZH2_ex4_ex14_cov, device = cairo_pdf, path = pdf_dir_fig2, 
       width = 16.5, height = 3.5, units = "cm")
```
...
```{r, echo = F, eval = TRUE}
rm(df_ezh2_ex4_wt, df_ezh2_ex14_wt)
```
...
```{r, Save flag rescues EZH2 exon 14 peptide coverage, message = F, fig.width = 6.5, fig.height = 1.8}
...
```{r, Plot flag rescues exon 4 14 peptide coverage, fig.width = 6, fig.height = 1.8}
plot_pep_cov(df_res = df_ezh2_ex14_flag, AA_exon_start = 83, AA_exon_end = 121) +
  labs(title = 'EZH2 exon 4 & exon 14 (FLAG-IP)') + 
  geom_vline(xintercept = 516, linewidth = 0.2) +
  geom_vline(xintercept = 558, linewidth = 0.2) -> p_EZH2_ex4_ex14_cov_flag
```

Save to pdf.

```{r, Save flag rescues exon 4 14 peptide coverage}
ggsave(filename = "Rev3_EZH2ex4-14_Rescues_IP_Peptides_Coverage.pdf", 
       plot = p_EZH2_ex4_ex14_cov_flag, device = cairo_pdf, path = pdf_dir_fig2, 
       width = 16.5, height = 3.5, units = "cm")
```
...
```{r, save all peptides coverage plots into 1 file}
